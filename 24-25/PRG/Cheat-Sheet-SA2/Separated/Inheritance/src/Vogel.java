// Implements: Das Interface "Tier" wird auf die Klasse "Vogel" angewandt. Die Klasse "Vogel" muss somit
// alle Methoden aus dem Interface "Tier" enthalten. Da "Vogel" abstract ist, würde kein Fehler auftreten,
...
// Somit ist es hier möglich sowohl das Interface "Tier" als auch das Interface "KannFliegen" und "HatFluegel" auf die
// Klasse "Vogel" anzuwenden.
...
public abstract class Vogel extends Lebewesen implements Tier, KannFliegen, HatFluegel {
    ...
    // Static: Die Variable ist unabhängig von der Instanz und ist für jede Instanz der Klasse Vogel gleich.
    // Das bedeutet egal auf welcher Instanz von "Vogel" die Variable "anzahl" verwendet wird,
    // befindet sich immer der gleiche Wert dahinter. Es wird gespeichert
    // wie viele Vögel instanziiert wurden.
    private static int anzahl;
    private double schnelligkeit;

    public Vogel(String name, double schnelligkeit){
        // Mit "super" wird die Elternklasse angesprochen. In diesem Fall der Konstruktor der Klasse "Lebewesen"
        super(name);
        this.schnelligkeit = schnelligkeit;
        anzahl++;
    }

    // Überladung einer Methode: Die Methode "bewegen" existiert bereits in der Elternklasse "Lebewesen".
    ...
    // "Vogel" kann dadurch beide Varianten aufrufen und verwenden. Siehe PatientTest.
